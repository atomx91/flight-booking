...
func (m *dbmanager) FindById(ctx context.Context, id uuid.UUID) (*booking_model.Booking, error) {
	...
	if err := m.Where(&booking_model.Booking{Id: id}).Preload("Client").Preload("Flight").First(&res).Error; err != nil {
		return nil, err
	}
...
func (m *dbmanager) SearchBooking(ctx context.Context, req *booking_request.SearchBookingRequest) ([]*booking_model.Booking, error) {
	...
	if err := m.Where(sbWhere, params...).Preload("Client").Preload("Flight").Find(&bookings).Error; err != nil {
		return nil, err
	}
