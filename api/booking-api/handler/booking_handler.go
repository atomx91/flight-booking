...
import (
	booking_request "flight-booking/api/booking-api/request"
	"flight-booking/pb"
	"math/rand"
	"net/http"
...
func (h *bookingHandler) ClientBooking(c *gin.Context) {
	req := booking_request.ClientBookingRequest{}
...
func (h *bookingHandler) GuestBooking(c *gin.Context) {
	...
	pReqCus := &pb.SearchClientRequest{
		Email:        req.Email,
		PhoneNumber:  req.PhoneNumber,
		IdentityCard: req.IdentityCard,
	}

	pResCus, err := h.clientClient.SearchClient(c.Request.Context(), pReqCus)
	...
	pClientId := ""
	if pResCus != nil && len(pResCus.Client) > 0 {
		// set client_id
		pClientId = pResCus.Client[0].Id
	} else {
		// If not existed => create new client
		pReqCreateCust := &pb.Client{
			Role:           0,
			Name:           req.Name,
			Email:          req.Email,
			PhoneNumber:    req.PhoneNumber,
		...
		pResCreateCust, err := h.clientClient.CreateClient(c.Request.Context(), pReqCreateCust)
		if err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
				"status": http.StatusText(http.StatusInternalServerError),
				"error":  err.Error(),
